---
lightbox: auto
title: "Token Caching for Diffusion Transformer Acceleration [`arxiv:2409.18523`](https://arxiv.org/pdf/2409.18523)"
date: 2024-10-16
...
- Use a small learnable network $g_{θ}$ dubbed **Cache Predictor** @lou2024tokencachingdiffusiontransformer to predict the importance of the tokens $w^{t}_{l} = g_{θ}(l, t) \in \mathbf{R}^{n}$, and prune the tokens based on their relative importance.
...
- How to learn $g_{θ}$?
  ...
  $$\hat{f}(z^{t}_{l}) = \textcolor{#800080}{w^{t}_{l} ⦿ f_{l}(z^{t}_{l})} + \textcolor{#008080}{(1 - w^{t}_{l}) ⦿ f_{l}(z^{t+1}_{l})}   $$
...
::: {.callout-important icon=false title="🔍 Optimization Objective" style="text-align:center;font-size:1.5em;width=100%"}
